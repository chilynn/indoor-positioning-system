...
public class ViewPhotoActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.tools_view_photo);

        photo_relativeLayout = (RelativeLayout) findViewById(R.id.photo_relativeLayout);
        photo_relativeLayout.setBackgroundColor(0x70000000);

        for (int i = 0; i < BitmapStore.bmp.size(); i++) {
            bmp.add(BitmapStore.bmp.get(i));
        }
        for (int i = 0; i < BitmapStore.drr.size(); i++) {
            drr.add(BitmapStore.drr.get(i));
        }
        max = BitmapStore.max;
        ...
        photo_bt_del.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                if (listViews.size() == 1) {
                    BitmapStore.bmp.clear();
                    BitmapStore.drr.clear();
                    BitmapStore.max = 0;
                    FileUtils.deleteDir();
                    finish();
                } else {
                    ...
                    drr.remove(count);
                    del.add(newStr);
                    BitmapStore.drr.remove(count);
                    BitmapStore.bmp.remove(count);
                    BitmapStore.max = BitmapStore.max - 1;
                    max--;
